...
int Interpreter::toInt(std::string str) {
	if (!isNum(str)) {
		std::cerr << "[ERROR] Not a num: " << str << std::endl;
		return -1;
	}
	try {
		return std::stoi(str);
		...
		std::cerr << "[ERROR] Num out of range: " << str << std::endl;
	}
	return -1;
}
...
void Interpreter::block(std::string name, std::vector<int> ios) {
	...
	Interpreter(newBlocks, exepath, path, lang, Echo).command("open " + filename);
	if (ios.size() != newBlocks.inputs.size() + newBlocks.outputs.size()) {
		std::cerr << "Incorrect line count" << std::endl;
		return;
	}
	ins = subVec(ios, 0, (int)newBlocks.inputs.size());
	outs = subVec(ios, (int)newBlocks.inputs.size(), (int)ios.size());
	for (int i : ins) {
		if (!assertInRange(i, blocks.L)) return;
		newBlocks.inputLines.push_back(&(blocks.L[i]));
	}
	for (int o : outs) {
...
void Interpreter::block_old(std::string name, std::vector<int> ios) {
	...
	ins = subVec(ios, 0, (int)blocks.Bs[blocks.Bs.size() - 1].inputs.size());
	outs = subVec(ios, (int)blocks.Bs[blocks.Bs.size() - 1].inputs.size(), (int)ios.size());
	for (int i : ins) {
		if (!assertInRange(i, blocks.L)) {
			blocks.Bs.pop_back();
			return;
		}
		blocks.Bs[blocks.Bs.size() - 1].inputLines.push_back(&(blocks.L[i]));
	}
	for (int o : outs) {
		...
		blocks.Bs[blocks.Bs.size() - 1].outputLines.push_back(&(blocks.L[o]));
	}
	writeMessage("BLOCK1", (int)blocks.Bs.size() - 1, name.c_str());
	for (int i : ins) {
		if (Echo) printf("%d ", i);
...
void Interpreter::__lang(std::string lan) {
	...
	else std::cerr << "Language not found: " << lan << std::endl;
}
...
void Interpreter::writeDebug() {
	...
	if (file == nullptr) {
		std::cerr << "[ERROR] Error writing debug.log\n";
	}
	else {
